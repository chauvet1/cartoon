...
import { Download, Sparkles, Upload, Settings, Layers, Zap } from "lucide-react"
...
export default function CartoonHero() {
  ...
  const [styleMix, setStyleMix] = useState({ primary: 'simpsons', secondary: 'anime', ratio: 100 })
  
  // UI State
  const [activeTab, setActiveTab] = useState<'single' | 'batch' | 'enhance' | 'customize'>('single')
  ...
  const handleBatchComplete = useCallback((results: any[]) => {
    addToast(`Batch processing completed! ${results.length} images processed.`, 'success');
  }, [addToast]);

  const handleEnhancedImage = useCallback((enhancedUrl: string) => {
    setEnhancedImage(enhancedUrl);
    setImage(enhancedUrl); // Update the main image with enhanced version
  }, []);

  const handleStyleIntensityChange = useCallback((intensity: number) => {
  ...
  const handleStyleMixChange = useCallback((mix: any) => {
    setStyleMix(mix);
  }, []);
    ...
    <div className="min-h-screen bg-[var(--color-neutral-50)]">
      <main className="w-full py-12 px-4">
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 mb-8">
          <div className="flex flex-wrap justify-center gap-2 bg-white rounded-xl p-2 shadow-sm border border-[var(--color-neutral-100)]">
            <Button
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          {activeTab === 'customize' && (
            <StyleCustomization
              ...
              onStyleChange={setCartoonStyle}
              onIntensityChange={handleStyleIntensityChange}
              onColorPaletteChange={handleColorPaletteChange}
              onStyleMixChange={handleStyleMixChange}
            />
          )}
        </div>
